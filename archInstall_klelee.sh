...
function partion {
	...
	case $is_partioning in 
		[yY][eE][sS]|[yY])
			...
			echo "And other partition is 'root' . "
			read -r -p "which drive you wang to partition (example /dev/sda)? " drive
			cfdisk $drive
			;;
		*)
			;;
...
function fs_format {
	read -r -p "Do you want to format you disk? This opration will clear your data![y/N] " is_format
	case $is_format in
		[yY][eE][sS]|[yY])
			lsblk
